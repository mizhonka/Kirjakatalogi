...
def my_books():
    sql = text(
        "SELECT b.id, b.title, b.author FROM Books b, Read r WHERE r.user_id=:id AND r.book_id=b.id")
...
def create(title, author, pub_year, lang, pagenumber, genres):
    ...
    result = db.session.execute(sql, {"title": title, "author": author,
                                "pub_year": pub_year, "lang": lang, "pagenumber": pagenumber})
    book_id = result.fetchone()[0]
    for genre in genres:
        sql = text(
            "INSERT INTO Genres (genre, book_id) VALUES (:genre, :book_id) RETURNING id")
...
def book_page(id):
    sql = text(
        "SELECT title, author, pub_year, lang, pagenumber FROM Books WHERE id=:id")
    ...
    result = db.session.execute(
        sql, {"id": id, "user_id": session["user_id"]}).fetchone()
    is_read = False
    if result:
        is_read = True
...
def mark_read(book_id):
    ...
    db.session.execute(
        sql, {"book_id": book_id, "user_id": session["user_id"]})
    db.session.commit()
...
def search_result(query, target):
    if target == "title":
        ...
        sql = text(
            "SELECT id, title, author FROM Books WHERE title LIKE :query OR author LIKE :query")
